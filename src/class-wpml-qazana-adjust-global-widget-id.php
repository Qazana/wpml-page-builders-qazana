...
class WPML_Qazana_Adjust_Global_Widget_ID {

	/** @var IWPML_Page_Builders_Data_Settings */
	private $qazana_settings;
	...
	public function __construct(
		IWPML_Page_Builders_Data_Settings $qazana_settings,
		WPML_Translation_Element_Factory $translation_element_factory,
		SitePress $sitepress
	) {
		$this->qazana_settings          = $qazana_settings;
		$this->translation_element_factory = $translation_element_factory;
		$this->sitepress                   = $sitepress;
	}

	public function add_hooks() {
		add_action( 'qazana/editor/before_enqueue_scripts', array( $this, 'adjust_ids' ) );
		add_action( 'qazana/editor/after_enqueue_scripts', array( $this, 'restore_current_language' ) );
	...
	public function adjust_ids() {
		...
		$custom_field_data = get_post_meta(
			$post_id,
			$this->qazana_settings->get_meta_field()
		);
		...
		$custom_field_data = $this->qazana_settings->convert_data_to_array( $custom_field_data );
		...
		if ( $custom_field_data_adjusted !== $custom_field_data ) {
			update_post_meta(
				$post_id,
				$this->qazana_settings->get_meta_field(),
				$this->qazana_settings->prepare_data_for_saving( $custom_field_data_adjusted )
			);

			// Update post date so Qazana doesn't use auto saved post
		...
		foreach ( $data_array as &$data ) {
			if ( isset( $data['elType'] ) && 'widget' === $data['elType'] && 'global' === $data['widgetType'] ) {
				try {
					...
					// Not much we can do if the qazana templateID is a non existing post
				}
			}
	...
	public function should_use_display_as_translated_snippet( $display_as_translated, $post_types ) {
		if ( isset( $_GET['action'] ) && 'qazana' === $_GET['action']
		     && in_array( 'qazana_library', array_keys( $post_types ), true ) ) {
			return true;
		}
