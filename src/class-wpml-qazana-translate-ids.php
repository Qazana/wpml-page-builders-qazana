...
class WPML_Qazana_Translate_IDs implements IWPML_Action {
	...
	public function add_hooks() {
		add_filter( 'qazana/theme/get_location_templates/template_id', array( $this, 'translate_theme_location_template_id' ) );
		add_filter( 'qazana/theme/get_location_templates/sub_id', array( $this, 'translate_location_condition_sub_id' ), 10, 2 );
		add_filter( 'qazana/documents/get/post_id', array(
			$this,
			'translate_template_id'
		) );
		add_filter( 'qazana/frontend/builder_content_data', array( $this, 'translate_global_widget_ids' ), 10, 2 );
	}
	...
	private function is_WP_widget_call() {
		return $this->debug_backtrace->is_class_function_in_call_stack(
			'QazanaPro\Modules\Library\WP_Widgets\Qazana_Library',
			'widget' );
	}

	private function is_shortcode_call() {
		return $this->debug_backtrace->is_class_function_in_call_stack(
			'QazanaPro\Modules\Library\Classes\Shortcode',
			'shortcode' );
	}

	private function is_template_widget_call() {
		return $this->debug_backtrace->is_class_function_in_call_stack(
			'QazanaPro\Modules\Library\Widgets\Template',
			'render' );
	}
