...
/**
 * Class Test_WPML_Qazana_Data_Settings
 *
 * @group page-builders
 * @group qazana
 */
class Test_WPML_Qazana_Data_Settings extends OTGS_TestCase {

	/**
	 * @test
	 */
	public function it_add_hooks() {
		$subject = new WPML_Qazana_Data_Settings();
	...
	public function it_add_hook_which_save_plain_text() {
		$qazana_db = $this->getMockBuilder( 'WPML_Qazana_DB' )
		                     ->disableOriginalConstructor()
		                     ->getMock();

		$subject = new WPML_Qazana_Data_Settings( $qazana_db );
	...
	public function it_marks_css_field_as_empty() {
		$subject = new WPML_Qazana_Data_Settings();
		$this->assertEquals( '', $subject->mark_css_field_as_empty( rand_str( 10 ), null, null, '_qazana_css' ) );
	}
	...
	public function it_gets_meta_field() {
		$subject = new WPML_Qazana_Data_Settings();
		$this->assertEquals( '_qazana_data', $subject->get_meta_field() );
	}
	...
	public function it_gets_field_to_copy() {
		$fields_to_copy = array( '_qazana_version', '_qazana_edit_mode', '_qazana_css', '_qazana_template_type', '_qazana_template_widget_type' );
	...
	public function it_gets_pb_name() {
		$subject = new WPML_Qazana_Data_Settings();
		$this->assertEquals( 'Qazana', $subject->get_pb_name() );
	}
	...
	public function it_gets_field_to_save() {
		$fields_to_copy = array( '_qazana_data' );
	...
	public function it_adds_custom_field() {
		...
		\WP_Mock::wpFunction( 'get_post_meta', array(
			'args'   => array( $post_id, '_qazana_data', true ),
			'return' => $pb_cf_value,
		) );
