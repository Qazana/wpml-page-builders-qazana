...
class Test_WPML_Qazana_Translate_IDs extends OTGS_TestCase {
	...
	public function it_adds_hooks() {
		...
		$subject = new WPML_Qazana_Translate_IDs( \Mockery::mock( 'WPML_Debug_BackTrace' ) );

		$this->expectFilterAdded( 'qazana/theme/get_location_templates/template_id', array(
			$subject,
			'translate_theme_location_template_id'
		), 10 );
		$this->expectFilterAdded( 'qazana/theme/get_location_templates/sub_id', array(
			$subject,
			'translate_location_condition_sub_id'
		), 10, 2 );
		$this->expectFilterAdded( 'qazana/documents/get/post_id', array(
			$subject,
			'translate_template_id'
		), 10 );
		$this->expectFilterAdded( 'qazana/frontend/builder_content_data', array(
			$subject,
	...
	public function it_should_translate_template_id_for_WP_widget() {
		...
		$debug_backtrace->shouldReceive( 'is_class_function_in_call_stack' )
		                ->with( 'QazanaPro\Modules\Library\WP_Widgets\Qazana_Library', 'widget' )
		                ->andReturn( true );

		$subject = new WPML_Qazana_Translate_IDs( $debug_backtrace );
	...
	public function it_should_translate_template_id_for_shortcode() {
		...
		$debug_backtrace->shouldReceive( 'is_class_function_in_call_stack' )
		                ...
		                ->andReturn( false );
		$debug_backtrace->shouldReceive( 'is_class_function_in_call_stack' )
		                ->with( 'QazanaPro\Modules\Library\Classes\Shortcode', 'shortcode' )
		                ->andReturn( true );
	...
	public function it_should_translate_template_id_for_template_widget() {
		...
		$debug_backtrace->shouldReceive( 'is_class_function_in_call_stack' )
		                ...
		                ->andReturn( false );
		$debug_backtrace->shouldReceive( 'is_class_function_in_call_stack' )
		                ->with( 'QazanaPro\Modules\Library\Widgets\Template', 'render' )
		                ->andReturn( true );
